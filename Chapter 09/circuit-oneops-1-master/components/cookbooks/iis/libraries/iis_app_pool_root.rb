...
module OO
  class IIS
    class AppPool
      class Root
        silence_warnings do
          PROPERTIES = %w{name managed_runtime_version managed_pipeline_mode enable32_bit_app_on_win64}
        ...
        def attributes
          attributes = {}
          PROPERTIES.each { |method_name| attributes[method_name] = @entity.send(method_name.camelize) }
